...
(function(){
  ...
  function auth(){
    pageEl.innerHTML=`<div class="tenant-auth-wrap"><form class="tenant-auth" id="tenantAuth"><div style="text-align:center"><span class="tenant-badge">🌐 도메인: ${esc(domain)}.creat1324.com</span></div><h1>선생님 인증</h1><p>학교에 등록된 교사 이메일을 입력해주세요.<br><small style="color:#94a3b8">학교 도메인 [${esc(domain)}]으로 접속되었습니다.</small></p><input id="tenantEmail" type="email" placeholder="teacher@school.com" required><button>인증하기</button><p id="tenantError" class="tenant-error"></p></form></div>`;
    document.getElementById('tenantAuth').onsubmit=verify;
  }
  async function verify(e){e.preventDefault();const email=document.getElementById('tenantEmail').value.trim().toLowerCase(),error=document.getElementById('tenantError');error.textContent='확인 중입니다...';if(!client){error.textContent='Supabase 연결 설정을 확인해주세요.';return}try{let r=await client.from('schools').select('id,name,department').eq('domain_name',domain).maybeSingle();if(r.error)throw r.error;if(!r.data){error.textContent='[ '+domain+' ] 학교 설정이 아직 등록되지 않았습니다. 관리자 페이지에서 학교 설정을 먼저 완료해 주세요.';return}school=r.data;r=await client.from('teachers').select('id,name,email').eq('school_id',school.id).ilike('email',email).maybeSingle();if(r.error)throw r.error;if(!r.data){error.textContent='등록되지 않은 교사 이메일입니다.';return}r=await client.from('students').select('id,name,student_number,country,photo_url,answers').eq('school_id',school.id);if(r.error)throw r.error;students=r.data||[];showClasses()}catch(err){console.error(err);error.textContent='인증 정보를 확인하지 못했습니다. 관리자에게 문의해주세요.'}}
